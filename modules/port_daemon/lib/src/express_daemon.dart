...
class ExpressDaemon extends Object with DatabaseHelpers implements PortDaemon {
  final HostMachine _hostMachine;
  final express.Express _express = new express.Express();
  final Logger _logger = new Logger('$ExpressDaemon');

  ExpressDaemon({HostMachine hostMachine})
      : _hostMachine = hostMachine ??
            createHostMachine(InternetAddress.LOOPBACK_IP_V4, 4369) {
    express.logger = (Object obj) {
      _logger.info(obj);
    };
    database = new NodeDatabase(new File('node.db'));
  }
